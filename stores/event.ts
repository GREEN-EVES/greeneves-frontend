	...
	persist(
		(set, get) => ({
			...
			updateEvent: async (id: string, data: Partial<Event>) => {
				console.log("data sent to API from event store", data);
				set({ isLoading: true });
				try {
				...
				try {
					...
					set((state) => {
						...
						return {
							...
							currentEventId:
								state.currentEventId === id ? newCurrentEvent?.id || null : state.currentEventId,
							isLoading: false,
						};
					});
				} catch (error) {
			...
			updateGuest: async (id: string, data: Partial<Guest>) => {
				try {
					...
					set((state) => ({
						guests: state.guests.map((guest) => (guest.id === id ? response.data.guest : guest)),
					}));
				} catch (error) {
					throw error;
				}
