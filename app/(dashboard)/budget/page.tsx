...
const budgetCategories = [
  ...
  'Music/DJ', 'Dress/Attire', 'Rings', 'Transportation', 'Decorations',
  'Invitations', 'Favors', 'Hair/Makeup', 'Honeymoon', 'Miscellaneous'
];
...
export default function BudgetTrackerPage() {
  ...
  const budgetStats = {
    ...
    percentageUsed: calculatedTotalBudget > 0 ? Math.round((budgetItems.reduce((sum, item) => sum + (item.actualAmount || 0), 0) / calculatedTotalBudget) * 100) : 0
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <p className="text-2xl font-bold text-foreground">₦{calculatedTotalBudget.toLocaleString()}</p>
                    <Button 
                      variant="ghost" 
                      size="sm" 
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-2xl font-bold text-red-600">₦{budgetStats.totalSpent.toLocaleString()}</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-2xl font-bold text-green-600">₦{budgetStats.remaining.toLocaleString()}</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-2xl font-bold text-purple-600">{budgetStats.percentageUsed}%</p>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>₦{budgetStats.totalSpent.toLocaleString()} spent of ₦{calculatedTotalBudget.toLocaleString()}</span>
                <span>{budgetStats.percentageUsed}% used</span>
              </div>
              <Progress 
                value={budgetStats.percentageUsed} 
                className="h-3"
                color={budgetStats.percentageUsed > 90 ? "red" : budgetStats.percentageUsed > 75 ? "yellow" : "green"}
              />
              ...
              <form onSubmit={(e) => {
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Current auto-calculated budget: ₦{budgetStats.totalBudgeted.toLocaleString()}
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button type="submit">Set Budget</Button>
                  <Button type="button" variant="outline" onClick={() => setShowBudgetEdit(false)}>
                    Cancel
                  </Button>
                  {totalBudget > 0 && (
                    <Button 
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-6 text-sm text-muted-foreground">
                            <span>
                              Budgeted: <span className="font-semibold">₦{(item.budgetedAmount || 0).toLocaleString()}</span>
                            </span>
                            <span>
                              Actual: <span className={`font-semibold ${(item.actualAmount || 0) > (item.budgetedAmount || 0) ? 'text-red-600' : 'text-green-600'}`}>
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="text-right mr-4">
                        <div className={`text-sm font-semibold ${
                          (item.actualAmount || 0) > (item.budgetedAmount || 0) ? 'text-red-600' : 
                          (item.actualAmount || 0) === 0 ? 'text-muted-foreground' : 'text-green-600'
                        }`}>
                          {(item.actualAmount || 0) > (item.budgetedAmount || 0) ? 
                            `+₦${((item.actualAmount || 0) - (item.budgetedAmount || 0)).toLocaleString()}` :
                            (item.actualAmount || 0) === 0 ? 'Not paid' :
                            `₦${((item.budgetedAmount || 0) - (item.actualAmount || 0)).toLocaleString()} saved`
                          }
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {(item.actualAmount || 0) > 0 && (item.budgetedAmount || 0) > 0 && `${Math.round(((item.actualAmount || 0) / (item.budgetedAmount || 0)) * 100)}% of budget`}
                        </div>
                      </div>
