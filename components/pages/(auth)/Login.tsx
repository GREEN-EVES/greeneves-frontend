...
export default function LoginPage() {
	...
	useEffect(() => {
		...
		if (redirect) {
			setRedirectPath(redirect === "event-setup" ? "/event-setup" : redirect);
		} else if (template) {
			setRedirectPath("/event-setup");
		}
	}, [searchParams]);
	...
	useEffect(() => {
		...
		if (isAuthenticated) {
			...
			if (templateId && redirectPath === "/event-setup") {
				finalRedirectPath = `/event-setup?template=${templateId}`;
			}
							...
							<div className="mt-6 text-center">
								<p className="text-sm text-gray-600">
									...
									<Link
										href={
											templateId
												? `/register?template=${templateId}&redirect=${redirectPath}`
												: "/register"
										}
